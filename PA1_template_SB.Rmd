...
what i want to do is create a matrix that is the means of each of the minute intervals and each of the weekdays.
...
```{r}
imputedf %>% filter(interval == 00:00:00 & wkday == 'Monday')
```
if my code is correct, 1.43 should be the answer for the nans.
...
mydata3 %>% 
      mutate(steps = ifelse(is.na(steps),
             imputedf$mean_steps[match(interval, imputedf$interval) &
                                       match(wkday, imputedf$wkday)], steps))
```
...
```{r}
mydata3 %>%
  mutate(steps = if_else(
    is.na(steps) &
    interval %in% imputedf$interval &
    wkday %in% imputedf$wkday,
    imputedf$mean_steps[match(interval, imputedf$interval) & match(wkday, imputedf$wkday)],
    steps
  ))
...
mydat %>% 
      ...
      pivot_wider(names_from = dayofweek, values_from = steps) %>% 
      mutate
```
