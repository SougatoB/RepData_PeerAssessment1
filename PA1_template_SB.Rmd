...
- It can be envisoned that the missing values for the steps taken for a particular 5min interval could be approximated reasonably well if we use the mean steps for 
      - that day of the week, and 
...
To accomplish this, we convert the dates to weekdays, group by weekday and interval, and get the corresponding means.
...
For a typical combination of wkday and interval, the steps taken can be predicted by mean_steps, as shown below.
...
We also add a weekday column to our original dataframe, so that we can join by weekday and interval.
...
- we carry out a left_join,
- impute missing values using values from the mean_steps column
- drop the mean_steps and wkday column to get back our imputed dataframe
...
```{r}
mydata3_imputed %>% filter(interval == 00:00:00 & date == '2012-10-01')
```
...
mydata4 %>% 
      ...
      summarize(meansteps = round(mean(steps, na.rm = TRUE), 2)) %>% 
...
this is the second part with imputed data to start with.
...
mydata5 %>% 
      group_by(interval, wknd) %>% 
      summarize(meansteps = round(mean(steps), 2)) %>% 
      ggplot(aes(interval, meansteps)) + geom_line(lwd = 1, 
                                                   color = 'steelblue') +
      facet_grid(wknd~.)
```
...
fulldata <- with_missing %>% 
      ...
      pivot_longer(3:4, names_to = 'miss_ornot', values_to = 'mean_vals') %>% 
      mutate(miss_ornot = as.factor(miss_ornot))
...
# use this
fulldata %>% 
      ggplot(aes(interval, mean_vals, color = miss_ornot)) +
      geom_line(lwd = 1) +
      facet_grid(wknd~.) +
...
- Some Observations:
  ...
  - The mean activity between 10AM and 4PM appears to be higher during the weekends, consistent with people being out and about instead of being at work.
  - During the weekends, people seem to be more active in the evenings after 8PM.
  - From 5-8AM, and 4-6PM, there is considerably more activity during the week compared to weekends; this is probably due to people getting ready to go to work or getting their exercise in the mornings/evenings, while more people are probably likely to sleep in during the weekend mornings.
